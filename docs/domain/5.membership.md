...
2. **One-to-many nominees** (Phase 1: enforce exactly 1 with priority=1)
...
- **Phase 1**: No existing nominee with priority = 1
...
- **Phase 1**: priority must = 1
- **Phase 1**: Only 1 nominee allowed
...
async function addNominee(input) {
  return await db.transaction(async (trx) => {
    ...
    // 2. Phase 1: Check if nominee already exists
    ...
    if (existingNominee) {
      throw new Error('Nominee with priority 1 already exists (Phase 1: only 1 nominee allowed)');
    }
    
    // 3. Validate priority
    if (input.nominee.priority !== 1) {
      throw new Error('Phase 1: Priority must be 1');
    }
...
- Member exists with registrationStatus = "Draft"
- registrationStep = "Nominees"
- **Phase 1**: Exactly 1 active nominee with priority = 1
...
async function completeNomineesStep(input) {
  return await db.transaction(async (trx) => {
    ...
    // 3. Phase 1: Validate exactly 1 nominee with priority 1
    const primaryNominee = nominees.find(n => n.priority === 1);
    if (!primaryNominee) {
      throw new Error('Primary nominee (priority 1) is required');
    }
    
    if (nominees.length > 1) {
      throw new Error('Phase 1: Only 1 nominee allowed');
    }
...
**Validations:**
1. **Nominees:**
   - Exactly 1 active nominee with priority = 1 (Phase 1)
2. **Documents:**
