...
export async function loginController(req, res, next) {
    try {
        ...
        const roles = userRoles.map((ur) => ur.role?.roleCode).filter(Boolean);
        const jwtPayload = {
            sub: subjectId,
            userId: subjectId,
            authUserId: user?.id,
 ...
 */
export async function checkAccessController(req, res, next) {
    try {
        ...
        const authContext = asyncLocalStorage.tryGetAuthContext();
        if (!authContext) {
            const result = { allowed: false, reason: 'Not authenticated' };
 ...
 */
async function checkResourceAccess(resource, resourceId, authContext) {
